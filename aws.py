...
class AWSClient:
    ...
    def _process_stream_event(
        ...
        if "validationException" in event_data:
            error_msg = event_data["validationException"].get(
        ...
        if "throttlingException" in event_data:
            error_msg = event_data["throttlingException"].get(
        ...
        if "contentBlockStart" in event_data:
            ...
            content_block_index = event_data["contentBlockStart"]["contentBlockIndex"]
        ...
        if "contentBlockDelta" in event_data:
            ...
            content_block_index = event_data["contentBlockDelta"]["contentBlockIndex"]
        ...
        if "messageStop" in event_data:
            ...
            return openai.types.chat.ChatCompletionChunk(
                ...
                choices=[
                    {
                        "delta": {},
                        "finish_reason": self._get_finish_reason(stop_reason),
                        "index": 0,
                    }
                ],
                created=int(time.time()),
